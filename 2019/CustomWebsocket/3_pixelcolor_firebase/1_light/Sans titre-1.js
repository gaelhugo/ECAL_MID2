...
wsServer.on('request', function(request){
  ...
  var connection = request.accept(null,request.origin); //conexion et identifiaction des devices
	var date = new Date();
  ...
  connection.on('message',function(message){ //message = string websocket
      ...
      switch(json.type){
        ...
        break;
        case "controllerMove":
          for(var name in clients){ //on lite les client connectés donc 2
            if(name != clientName){ //si meme renvoie pas
